  ...
  str.each_char do |char|
    ...
    if opening_chars.include?(char) # opening bracket
      ...
      if char == required_char # it's the right kind of closing bracket
        ...
        if open.any?
          sub_var = openables[open.last] # it's the wrong kind of closing bracket
          puts "* You have a syntax error: there is an unexpected #{char} where there should be a #{sub_var}"
